...
export default function Modal({ isOpen, onClose, title, children, size = 'md' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
      ...
      <div className={`relative max-h-[94vh] w-full overflow-y-auto rounded-lg bg-white shadow-2xl dark:bg-slate-900 ${sizes[size]}`}>
        ...
        <div className="p-4 text-gray-800 dark:text-slate-100 sm:p-6">{children}</div>
      </div>
    </div>
  );
}
